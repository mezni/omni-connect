...
Generates curated markdown knowledge documents (guidelines, policies, processes,
eligibility terms, terms & conditions) for the omni-connect telecom retail domain.

Output mirrors the bootcamp knowledge_base layout:
    data/knowledge_base/<topic>_<type>.md.txt
"""
...
def doc(
    *,
    ...
    """Convenience builder that registers a KnowledgeDocument with its output path."""
    return {
        "filename": filename,
...
# ==============================================================================
# 2. CURATED KNOWLEDGE DOCUMENTS (telecom retail domain)
# ==============================================================================
...
KB = [
    ...
    # --------------------------------------------------------------------------
    doc(
        filename="postpaid_plan_guidelines.md.txt",
...
*Plans and pricing subject to change. Terms and conditions apply.*""",
    ...
    doc(
        filename="plan_upgrade_policy.md.txt",
...
*Terms and conditions apply.*""",
    ...
    doc(
        filename="data_plan_terms_conditions.md.txt",
...
*Terms subject to regulatory change.*""",
    ...
    # --------------------------------------------------------------------------
    doc(
        filename="device_upgrade_guidelines.md.txt",
...
*Terms and conditions apply.*""",
    ...
    doc(
        filename="device_trade_in_process.md.txt",
...
*Valuations subject to physical inspection.*""",
    ...
    doc(
        filename="device_protection_policy.md.txt",
...
*Deductibles and limits subject to change.*""",
    ...
    # --------------------------------------------------------------------------
    doc(
        filename="billing_invoice_policy.md.txt",
...
*Billing terms apply.*""",
    ...
    doc(
        filename="payment_eligibility_terms.md.txt",
...
*Payment terms apply.*""",
    ...
    # --------------------------------------------------------------------------
    doc(
        filename="port_in_policy.md.txt",
...
*Ports subject to carrier schedules.*""",
    ...
    doc(
        filename="new_line_activation_process.md.txt",
...
*Activation terms apply.*""",
    ...
    doc(
        filename="returns_refunds_policy.md.txt",
...
*Returns policy applies.*""",
    ...
    # --------------------------------------------------------------------------
    doc(
        filename="promotions_eligibility_terms.md.txt",
...
def main() -> None:
    ...
    for entry in KB:
        path = knowledge_dir / entry["filename"]
        path.write_text(entry["document"].rendered, encoding="utf-8")

    print(f"Successfully generated knowledge base under '{knowledge_dir.resolve()}':")
    for entry in KB:
